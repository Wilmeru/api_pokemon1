...
function App(){
 ...
 const buscar=async()=>{
    if(!busqueda.trim()){
        setError("Escribe un Pokémon");
        return}
        try{setCargando(true);setError("");setPokemon(null);
            const r=await fetch("https://pokeapi.co/api/v2/pokemon/"+busqueda.trim().toLowerCase());
            if(!r.ok)throw new Error("Pokémon no encontrado");
            const d=await r.json();
            const ataque=d.stats.find(s=>s.stat.name==="attack")?.base_stat??0;
